...
class ArnavCfg {
    ...
    peekCfgParam(docId, paramName) {
		...
		if(this.params.doc[docId])
			paramValue = 
				useful(this.params.doc[docId][paramName] ? 
					this.params.doc[docId][paramName] : this.params.sys[paramName], "");
	...
	unpackFromJson(paramsJson) {
		...
		try {
			...
			parseSuccess = !!params ? parseSuccess = params.sys && params.doc : false;
		}
		catch(e) {
			parseSuccess = false;
		}			
